...
func detectStorageDisksGo() map[string]interface{} {
	...
	var eligible, nvme, usb, provisioned, orphan []interface{}

	// HARD-4: snapshot del observer para reclasificar discos con BTRFS huérfano.
	// Si globalObserver no está listo, snap es nil y la lógica cae a `eligible`
	// (comportamiento previo). preFlightCheck es la red de seguridad real;
	// esto es UX: evitar que el usuario seleccione un disco con FS huérfano
	// pensando que está vacío.
	var observerSnap *ObservedSnapshot
	if globalObserver != nil {
		observerSnap = globalObserver.Snapshot()
	}
	...
	for _, raw := range data.BlockDevices {
		...
		// HARD-4: ¿hay un BTRFS huérfano que reclama este disco?
		// Lo capturamos antes de "eligible" para que la UI no proponga
		// destruir un FS que el usuario podría querer importar.
		if orphanFS := findOrphanFilesystemForDisk(observerSnap, devName); orphanFS != nil {
			diskInfo["classification"] = "orphan_filesystem"
			diskInfo["orphanFilesystem"] = map[string]interface{}{
				"uuid":  orphanFS.UUID,
				"label": orphanFS.Label,
			}
			orphan = append(orphan, diskInfo)
			continue
		}
	...
	if orphan == nil { orphan = []interface{}{} }

	result["eligible"] = eligible
	result["nvme"] = nvme
	result["usb"] = usb
	result["provisioned"] = provisioned
	result["orphan_filesystem"] = orphan
	return result
}
