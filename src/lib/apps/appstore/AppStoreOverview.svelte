...
>
  ...
  {:else}
    <div class="overview">
      <!-- Header con título dinámico + counts + search inline -->
      <div class="overview-head">
        <h1 class="head-title">{activeLabel}</h1>
        <div class="head-meta">
          {visibleViews.length} de {allViews.length}
          {#if installed.length > 0} · {installed.length} instalada{installed.length === 1 ? '' : 's'}{/if}
        </div>
