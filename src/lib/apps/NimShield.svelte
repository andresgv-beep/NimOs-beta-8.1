<script>
  ...
  async function refresh() {
    const [s, e, b, wl, rep, cfg] = await Promise.all([
      get('status'), get('events?limit=200'), get('blocks'), get('whitelist'), get('reputation'), get('config'),
    ]);
    if (s) status = s;
    if (e) events = e.events || [];
    if (b) blocks = (b.blocks || []).sort((x, y) => (x.expiresAt < y.expiresAt ? -1 : 1));
    if (wl) whitelist = wl.whitelist || [];
    if (rep) reputation = rep.reputation || [];
    if (cfg) {
      config = cfg.config;
      configDefaults = cfg.defaults;
      if (!configDraft) configDraft = { ...cfg.config };
    }
    loading = false;
  }
  ...
  async function saveConfig() {
    if (configSaving || !configDraft) return;
    configSaving = true; configMsg = '';
    try {
      const r = await put('config', configDraft);
      config = r.config;
      configDraft = { ...r.config };
      configMsg = 'Guardado ✓';
  ...
  function restoreDefaults() {
    if (configDefaults) configDraft = { ...configDefaults };
  }
  ...
  // ─── Acciones bloqueos ───
  async function forgetReputation(ip) {
    if (!confirm(`¿Olvidar la reputación de ${ip}? Volverá a trato estricto (desconocida).`)) return;
    busy = new Set(busy).add(ip);
    try {
      await post('reputation/forget', { ip });
      await refresh();
    } catch (e) {
      // silencioso: el refresh reflejará el estado real
    } finally {
      busy.delete(ip); busy = new Set(busy);
    }
  }

  // etiqueta + clase del nivel de reputación
  const repLevelMeta = {
        ...
        {:else}
          {#each reputation as rp (rp.ip)}
            <div class="block-row rep-row">
              ...
              <span class="block-created">{rp.lastSuccess ? fmtAgo(rp.lastSuccess, true) : '—'}</span>
              <div class="block-actions" style="justify-content:flex-end">
                <button class="icon-btn" title="Olvidar reputación (vuelve a trato estricto)" disabled={busy.has(rp.ip)} on:click={() => forgetReputation(rp.ip)}>
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
        ...
        <div class="set-wrap">
          <div class="set-group">
            ...
            <div class="set-row">
              ...
              <input type="number" min="1" max="100" bind:value={configDraft.fail_unknown} />
              <span class="set-unit">fallos</span>
            </div>
            <div class="set-row">
              <label>IP conocida</label>
              <input type="number" min="1" max="100" bind:value={configDraft.fail_known} />
              <span class="set-unit">fallos</span>
            </div>
            <div class="set-row">
              <label>IP habitual</label>
              <input type="number" min="1" max="100" bind:value={configDraft.fail_habitual} />
              <span class="set-unit">fallos</span>
            </div>
            <div class="set-row">
              <label>Racha de desconfianza</label>
              <input type="number" min="2" max="20" bind:value={configDraft.distrust_streak} />
          ...
          <div class="set-group">
            ...
            <div class="set-row">
              ...
              <input type="number" min="1" max="1440" bind:value={configDraft.block_min_1} />
              <span class="set-unit">min</span>
            </div>
            <div class="set-row">
              <label>2º bloqueo</label>
              <input type="number" min="1" max="1440" bind:value={configDraft.block_min_2} />
              <span class="set-unit">min</span>
            </div>
            <div class="set-row">
              <label>3er bloqueo</label>
              <input type="number" min="1" max="1440" bind:value={configDraft.block_min_3} />
              <span class="set-unit">min</span>
            </div>
            <div class="set-row">
              <label>4º en adelante</label>
              <input type="number" min="1" max="1440" bind:value={configDraft.block_min_4} />
              <span class="set-unit">min</span>
            </div>
          </div>
          ...
          <div class="set-actions">
            {#if configMsg}<span class="set-msg" class:err={configMsg !== 'Guardado ✓'}>{configMsg}</span>{/if}
            <button class="set-btn ghost" on:click={restoreDefaults} disabled={configSaving}>Restaurar por defecto</button>
            <button class="set-btn" on:click={saveConfig} disabled={configSaving}>{configSaving ? 'Guardando…' : 'Guardar'}</button>
          </div>
        </div>
      {:else}
...
<style>
  ...
  .rep-num {
    ...
    color: var(--fg-3, #b0b0b8); font-variant-numeric: tabular-nums;
  }
  .rep-streak { color: var(--st-warn, #ffc857); font-weight: 600; }
  .lvl-pill {
    display: inline-block; font-family: var(--font-mono); font-size: 10px;
    padding: 2px 8px; border-radius: 99px; border: 1px solid var(--bd-3, #2a2a32);
    color: var(--fg-4, #7a7a82); letter-spacing: 0.3px;
  }
  ...
  .set-group {
    border: 1px solid var(--bd-2, #20202a); border-radius: 8px;
    background: var(--bg-inner, #101015); padding: 16px 18px;
  }
  .set-group-head {
    font-family: var(--font-mono); font-size: 11px; letter-spacing: 0.6px;
    text-transform: uppercase; color: var(--nim-green, #00ff9f); margin-bottom: 6px;
  }
  ...
  .set-row input {
    width: 80px; padding: 6px 10px; text-align: right;
    background: var(--bg, #0a0a0d); border: 1px solid var(--bd-3, #2a2a32);
    border-radius: 6px; color: var(--fg, #f0f0f0);
  ...
  .set-btn {
    padding: 8px 18px; border-radius: 6px; cursor: pointer; font-size: 12.5px; font-weight: 600;
    border: 1px solid var(--nim-green, #00ff9f); background: rgba(0,255,159,0.1); color: var(--nim-green, #00ff9f);
  }
