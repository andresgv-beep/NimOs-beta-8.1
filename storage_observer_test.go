...
// ─── findOrphanFilesystemForDisk ──────────────────────────────────────────

func TestFindOrphanFilesystemForDisk_Hit(t *testing.T) {
	snap := &ObservedSnapshot{Filesystems: []ObservedBtrfs{
		{UUID: "uuid-orphan", Label: "old-pool", IsManaged: false,
			Devices: []ObservedDevice{{Path: "/dev/sdb"}}},
	}}
	fs := findOrphanFilesystemForDisk(snap, "sdb")
	if fs == nil || fs.UUID != "uuid-orphan" {
		t.Fatalf("expected hit on uuid-orphan, got %+v", fs)
	}
}

func TestFindOrphanFilesystemForDisk_SkipsManaged(t *testing.T) {
	snap := &ObservedSnapshot{Filesystems: []ObservedBtrfs{
		{UUID: "uuid-managed", IsManaged: true,
			Devices: []ObservedDevice{{Path: "/dev/sdb"}}},
	}}
	if fs := findOrphanFilesystemForDisk(snap, "sdb"); fs != nil {
		t.Fatalf("managed FS no debe devolverse, got %+v", fs)
	}
}

func TestFindOrphanFilesystemForDisk_NilOrMiss(t *testing.T) {
	if fs := findOrphanFilesystemForDisk(nil, "sdb"); fs != nil {
		t.Errorf("nil snapshot debe devolver nil, got %+v", fs)
	}
	snap := &ObservedSnapshot{Filesystems: []ObservedBtrfs{
		{UUID: "uuid-other", IsManaged: false,
			Devices: []ObservedDevice{{Path: "/dev/sdc"}}},
	}}
	if fs := findOrphanFilesystemForDisk(snap, "sdb"); fs != nil {
		t.Errorf("disk no presente debe devolver nil, got %+v", fs)
	}
}
