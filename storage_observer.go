...
// findOrphanFilesystemForDisk busca en el snapshot un filesystem BTRFS NO managed
// que contenga un device cuyo path coincida con /dev/<devName>.
//
// Devuelve el filesystem orphan si lo encuentra, o nil:
//   - si snap es nil
//   - si devName no aparece en ningún FS
//   - si el FS que lo contiene es managed (cubierto por un pool de NimOS)
//
// Función pura: no toca disco, no toca globals. Pensada para que callers
// (ej: detectStorageDisksGo) puedan reclasificar discos eligible que en
// realidad pertenecen a un filesystem huérfano.
func findOrphanFilesystemForDisk(snap *ObservedSnapshot, devName string) *ObservedBtrfs {
	if snap == nil || devName == "" {
		return nil
	}
	target := "/dev/" + devName
