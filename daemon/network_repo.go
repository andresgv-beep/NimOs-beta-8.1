...
//
// Encapsula el acceso a las tablas network_ports, network_ddns,
// network_certs. Estas son las 3 tablas con TRIPLE GENERATION
...
var (
	...
	ErrCertNotFound = errors.New("network cert not found")
...
// NetworkCert representa una fila de network_certs.
...
const certColumns = `
	id, domain, cert_provider, challenge_type, dns_provider,
	fullchain_path, privkey_path,
	not_before, not_after,
	enabled, auto_renew,
	issued_at, last_renewed_at,
	desired_generation, observed_generation, applied_generation
`

func scanCert(rs rowScanner) (*NetworkCert, error) {
	var (
		c               NetworkCert
		challengeType   sql.NullString
		dnsProvider     sql.NullString
		notBeforeStr    string
		notAfterStr     string
		enabledInt      int
		autoRenewInt    int
	...
	err := rs.Scan(
		&c.ID, &c.Domain, &c.CertProvider, &challengeType, &dnsProvider,
		&c.FullchainPath, &c.PrivkeyPath,
		&notBeforeStr, &notAfterStr,
		&enabledInt, &autoRenewInt,
		&issuedAtStr, &lastRenewedAt,
		&c.Convergence.Desired, &c.Convergence.Observed, &c.Convergence.Applied,
	)
	if err != nil {
		return nil, err
	}
	c.ChallengeType = ptrFromNullString(challengeType)
	c.DNSProvider = ptrFromNullString(dnsProvider)
	c.NotBefore = parseTime(notBeforeStr)
	c.NotAfter = parseTime(notAfterStr)
	c.Enabled = boolFromInt(enabledInt)
	c.AutoRenew = boolFromInt(autoRenewInt)
	c.IssuedAt = parseTime(issuedAtStr)
	c.LastRenewedAt = ptrFromNullTime(lastRenewedAt)
	return &c, nil
}

// CreateCert inserta un certificado. El schema valida la coherencia
// challenge_type ↔ dns_provider con un CHECK; si es incoherente, el
// INSERT falla.
//
// Si el ID está vacío, se genera un UUID.
// issued_at por defecto = clock.Now().
func (r *NetworkRepo) CreateCert(ctx context.Context, tx *sql.Tx, c *NetworkCert) error {
	if c.ID == "" {
		c.ID = uuid.New().String()
	}
	if c.IssuedAt.IsZero() {
		c.IssuedAt = r.clock.Now().UTC()
	}
	_, err := tx.ExecContext(ctx, `
		INSERT INTO network_certs (
			id, domain, cert_provider, challenge_type, dns_provider,
			fullchain_path, privkey_path,
			not_before, not_after,
			enabled, auto_renew,
	...
	`,
		c.ID, c.Domain, c.CertProvider, nullStringPtr(c.ChallengeType), nullStringPtr(c.DNSProvider),
		c.FullchainPath, c.PrivkeyPath,
		c.NotBefore.UTC().Format(time.RFC3339), c.NotAfter.UTC().Format(time.RFC3339),
		intFromBool(c.Enabled), intFromBool(c.AutoRenew),
		c.IssuedAt.UTC().Format(time.RFC3339),
	)
	if err != nil {
		if isUniqueConstraintError(err) {
			return fmt.Errorf("cert domain %q already exists: %w", c.Domain, err)
		}
		return fmt.Errorf("CreateCert: %w", err)
	}
	c.Convergence = Convergence{Desired: 1, Observed: 0, Applied: 0}
	return nil
}

// GetCert lee un certificado por id.
func (r *NetworkRepo) GetCert(ctx context.Context, id string) (*NetworkCert, error) {
	row := r.db.QueryRowContext(ctx,
		`SELECT `+certColumns+` FROM network_certs WHERE id = ?`, id)
	c, err := scanCert(row)
	if errors.Is(err, sql.ErrNoRows) {
...
// GetCertByDomain lee un certificado por domain (UNIQUE).
func (r *NetworkRepo) GetCertByDomain(ctx context.Context, domain string) (*NetworkCert, error) {
	row := r.db.QueryRowContext(ctx,
		`SELECT `+certColumns+` FROM network_certs WHERE domain = ?`, domain)
	c, err := scanCert(row)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, ErrCertNotFound
	}
	if err != nil {
		return nil, fmt.Errorf("GetCertByDomain: %w", err)
	}
	return c, nil
}

// ListCerts devuelve todos los certificados ordenados por domain.
func (r *NetworkRepo) ListCerts(ctx context.Context) ([]*NetworkCert, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT `+certColumns+` FROM network_certs ORDER BY domain`)
	if err != nil {
		return nil, fmt.Errorf("ListCerts: %w", err)
	}
	defer rows.Close()
	return collectCerts(rows)
}

// ListExpiringCerts devuelve los certificados auto_renew=1 que expiran
// en menos de `within` (típicamente 30d para renovación preventiva).
func (r *NetworkRepo) ListExpiringCerts(ctx context.Context, within time.Duration) ([]*NetworkCert, error) {
	threshold := r.clock.Now().UTC().Add(within).Format(time.RFC3339)
	rows, err := r.db.QueryContext(ctx, `
		SELECT `+certColumns+`
		FROM network_certs
		WHERE auto_renew = 1 AND not_after < ?
		ORDER BY not_after ASC
	`, threshold)
	if err != nil {
		return nil, fmt.Errorf("ListExpiringCerts: %w", err)
	}
	defer rows.Close()
	return collectCerts(rows)
}

// UpdateCertRenewed actualiza paths, fechas y last_renewed_at tras una
// renovación exitosa. NO toca generations — usar MarkCertApplied después.
func (r *NetworkRepo) UpdateCertRenewed(ctx context.Context, tx *sql.Tx, id, fullchainPath, privkeyPath string, notBefore, notAfter time.Time) error {
	now := r.clock.Now().UTC().Format(time.RFC3339)
	res, err := tx.ExecContext(ctx, `
		UPDATE network_certs
		SET fullchain_path = ?, privkey_path = ?,
		    not_before = ?, not_after = ?,
		    last_renewed_at = ?
		WHERE id = ?
	`,
	...
	if err != nil {
		return fmt.Errorf("UpdateCertRenewed: %w", err)
	}
	n, _ := res.RowsAffected()
	if n == 0 {
...
// SetCertAutoRenew habilita/deshabilita la renovación automática.
// Incrementa desired_generation porque es un cambio de policy.
func (r *NetworkRepo) SetCertAutoRenew(ctx context.Context, tx *sql.Tx, id string, autoRenew bool) error {
	res, err := tx.ExecContext(ctx, `
		UPDATE network_certs
		SET auto_renew = ?,
		    desired_generation = desired_generation + 1
		WHERE id = ?
	`, intFromBool(autoRenew), id)
	if err != nil {
		return fmt.Errorf("SetCertAutoRenew: %w", err)
	}
	n, _ := res.RowsAffected()
	if n == 0 {
		return ErrCertNotFound
...
// RecordCertObserved incrementa observed_generation (drift detectado:
// el cert en disco no coincide con el applied).
func (r *NetworkRepo) RecordCertObserved(ctx context.Context, tx *sql.Tx, id string) error {
	res, err := tx.ExecContext(ctx, `
		UPDATE network_certs
		SET observed_generation = observed_generation + 1
		WHERE id = ?
	`, id)
	if err != nil {
		return fmt.Errorf("RecordCertObserved: %w", err)
	}
	n, _ := res.RowsAffected()
	if n == 0 {
		return ErrCertNotFound
...
// MarkCertApplied: applied_generation = desired_generation. También
// sincroniza observed_generation — ver razón en MarkPortApplied.
func (r *NetworkRepo) MarkCertApplied(ctx context.Context, tx *sql.Tx, id string) error {
	res, err := tx.ExecContext(ctx, `
		UPDATE network_certs
		SET applied_generation  = desired_generation,
		    observed_generation = desired_generation
	...
	`, id)
	if err != nil {
		return fmt.Errorf("MarkCertApplied: %w", err)
	}
	n, _ := res.RowsAffected()
	if n == 0 {
		return ErrCertNotFound
...
// DeleteCert borra el registro. NO borra los archivos de disco
// (responsabilidad del caller, ej: el reconciler que orquesta la
// eliminación completa).
func (r *NetworkRepo) DeleteCert(ctx context.Context, tx *sql.Tx, id string) error {
	_, err := tx.ExecContext(ctx, `DELETE FROM network_certs WHERE id = ?`, id)
	if err != nil {
		return fmt.Errorf("DeleteCert: %w", err)
	}
	return nil
}

// ListPendingCerts: applied < desired (cambio de config sin aplicar).
func (r *NetworkRepo) ListPendingCerts(ctx context.Context) ([]*NetworkCert, error) {
	rows, err := r.db.QueryContext(ctx, `
		SELECT `+certColumns+`
		FROM network_certs
		WHERE applied_generation < desired_generation AND enabled = 1
	...
	`)
	if err != nil {
		return nil, fmt.Errorf("ListPendingCerts: %w", err)
	}
	defer rows.Close()
	return collectCerts(rows)
}

// ListDriftedCerts: observed != applied.
func (r *NetworkRepo) ListDriftedCerts(ctx context.Context) ([]*NetworkCert, error) {
	rows, err := r.db.QueryContext(ctx, `
		SELECT `+certColumns+`
		FROM network_certs
		WHERE observed_generation != applied_generation AND enabled = 1
	...
	`)
	if err != nil {
		return nil, fmt.Errorf("ListDriftedCerts: %w", err)
	}
	defer rows.Close()
	return collectCerts(rows)
}

func collectCerts(rows *sql.Rows) ([]*NetworkCert, error) {
	var out []*NetworkCert
	for rows.Next() {
		c, err := scanCert(rows)
		if err != nil {
