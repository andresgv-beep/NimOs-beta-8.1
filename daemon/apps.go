...
// Cruza docker_apps (config persistente) con docker ps (runtime).
// Usado por /api/services para NimHealth.
// ═══════════════════════════════════════════════════════════════════════

// dockerContainer es una línea parseada de docker ps -a
...
// getDockerAppStatuses construye DockerAppStatus para cada app registrada
// cruzando docker_apps (SQLite) con docker ps -a.
...
func getDockerAppStatuses(dockerServiceID string) ([]DockerAppStatus, int) {
	...
	for _, reg := range registered {
		// Buscar container · prueba exact match y prefijos de stack
		...
		// Fallback: prefix match para stacks (immich_server → immich)
		if found == nil {
		...
		if found != nil {
			...
			status.Health = "idle"
			// Usar el puerto declarado de la config
			if reg.Port > 0 {
	...
	// 4. Contar containers huérfanos (en docker ps pero no en docker_apps)
	orphanCount := 0
...
// extractUptime obtiene la duración de docker ps STATUS, e.g. "Up 3 hours" → "3h"
func extractUptime(rawStatus string) string {
	...
	// Compactar formato: "3 hours" → "3h", "2 days" → "2d", "About a minute" → "1m"
...
// parsePorts extrae bindings de docker ps PORTS string, mergeando con config.
//
// Acepta el config como *DBDockerApp para leer el `port` principal.
// Si docker no reporta puertos pero la config tiene uno, usamos ese como fallback.
func parsePorts(rawPorts string, config *DBDockerApp) []PortBinding {
	if rawPorts == "" {
		// Fallback al puerto del config
	...
	// Format: "0.0.0.0:8096->8096/tcp, :::8096->8096/tcp"
