...
-- =============================================================================
-- 3. network_certs — Certificados TLS
-- =============================================================================
-- challenge_type y dns_provider son nullable porque:
--   · selfsigned: ambos NULL (no hay challenge ni DNS challenge).
--   · letsencrypt + http-01: challenge_type='http-01', dns_provider=NULL.
--   · letsencrypt + dns-01:  challenge_type='dns-01',  dns_provider=...
--
-- El CHECK verifica la coherencia: si challenge_type='dns-01', el
-- dns_provider DEBE estar; si es 'http-01' o NULL, el dns_provider DEBE
-- ser NULL. Esto evita configs corruptas a nivel DB.
...
CREATE TABLE IF NOT EXISTS network_certs (
    ...
    -- Coherencia challenge_type ↔ dns_provider (table-level, al final).
    CHECK (
        (challenge_type IS NULL     AND dns_provider IS NULL) OR
        (challenge_type = 'http-01' AND dns_provider IS NULL) OR
        (challenge_type = 'dns-01'  AND dns_provider IS NOT NULL)
    )
);

CREATE INDEX IF NOT EXISTS idx_network_certs_not_after  ON network_certs(not_after);
CREATE INDEX IF NOT EXISTS idx_network_certs_auto_renew ON network_certs(auto_renew) WHERE auto_renew = 1;
...
CREATE TABLE IF NOT EXISTS network_observed (
    ...
    certs_total      INTEGER NOT NULL DEFAULT 0 CHECK(certs_total      >= 0),
    certs_expiring   INTEGER NOT NULL DEFAULT 0 CHECK(certs_expiring   >= 0),
