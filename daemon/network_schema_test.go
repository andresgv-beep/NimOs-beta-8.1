...
// ─────────────────────────────────────────────────────────────────────────────
// network_certs CHECKs (incluido el coherence challenge ↔ dns_provider)
// ─────────────────────────────────────────────────────────────────────────────

func TestNetworkSchema_CertsSelfSignedAllowsNullChallengeAndDNS(t *testing.T) {
	c, cleanup := setupNetworkDB(t)
	defer cleanup()

	_, err := c.db.Exec(`
		INSERT INTO network_certs (
			id, domain, cert_provider, fullchain_path, privkey_path,
			not_before, not_after, issued_at
		) VALUES (
			'cert-1', 'host.local', 'selfsigned', '/etc/ssl/full.pem', '/etc/ssl/key.pem',
			'2026-05-21T12:00:00Z', '2027-05-21T12:00:00Z', '2026-05-21T12:00:00Z'
		)
	`)
	if err != nil {
		t.Errorf("self-signed with NULL challenge+dns failed: %v", err)
	}
}

func TestNetworkSchema_CertsHTTP01RejectsDNSProvider(t *testing.T) {
	c, cleanup := setupNetworkDB(t)
	defer cleanup()
	...
	_, err := c.db.Exec(`
		INSERT INTO network_certs (
			...
			fullchain_path, privkey_path, not_before, not_after, issued_at
		) VALUES (
			'cert-1', 'host.example.com', 'letsencrypt', 'http-01', 'duckdns',
			'/etc/ssl/full.pem', '/etc/ssl/key.pem',
			'2026-05-21T12:00:00Z', '2027-05-21T12:00:00Z', '2026-05-21T12:00:00Z'
		)
	`)
	if err == nil {
		t.Error("CHECK should reject http-01 with non-null dns_provider (incoherent)")
	}
}

func TestNetworkSchema_CertsDNS01RequiresDNSProvider(t *testing.T) {
	c, cleanup := setupNetworkDB(t)
	defer cleanup()
	...
	_, err := c.db.Exec(`
		INSERT INTO network_certs (
			...
			'cert-1', 'host.example.com', 'letsencrypt', 'dns-01', NULL,
			'/etc/ssl/full.pem', '/etc/ssl/key.pem',
			'2026-05-21T12:00:00Z', '2027-05-21T12:00:00Z', '2026-05-21T12:00:00Z'
		)
	`)
	if err == nil {
		t.Error("CHECK should reject dns-01 with NULL dns_provider (incoherent)")
	}
}

func TestNetworkSchema_CertsDNS01WithProviderOK(t *testing.T) {
	c, cleanup := setupNetworkDB(t)
	defer cleanup()
	...
	_, err := c.db.Exec(`
		INSERT INTO network_certs (
			...
			'cert-1', 'host.duckdns.org', 'letsencrypt', 'dns-01', 'duckdns',
			'/etc/ssl/full.pem', '/etc/ssl/key.pem',
			'2026-05-21T12:00:00Z', '2027-05-21T12:00:00Z', '2026-05-21T12:00:00Z'
		)
	`)
	if err != nil {
		t.Errorf("valid dns-01 + duckdns insert failed: %v", err)
	}
}

func TestNetworkSchema_CertsUniqueDomain(t *testing.T) {
	c, cleanup := setupNetworkDB(t)
	defer cleanup()
...
func TestNetworkSchema_EventsAcceptsAllCategories(t *testing.T) {
	...
	for _, cat := range []string{"ddns", "cert", "port", "upnp", "breaker", "observer", "capability"} {
		_, err := c.db.Exec(`
