...
func (m *mockNetworkProbe) Probe(_ []CertProbeInput, _ []PortProbeInput) ProbeResult {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.calls++
	return m.result
...
func TestObserver_NewAppliesDefaults(t *testing.T) {
	...
	if obs.config.ExpiringWindow == 0 {
		t.Error("ExpiringWindow default not applied")
	}
}
...
func seedAppliedCert(t *testing.T, db *sql.DB, repo *NetworkRepo, domain string, notAfter time.Time) string {
	t.Helper()
	cert := &NetworkCert{
		Domain: domain, CertProvider: "selfsigned",
		FullchainPath: "/etc/ssl/" + domain + "/fullchain.pem",
		PrivkeyPath:   "/etc/ssl/" + domain + "/privkey.pem",
	...
	withNetTx(t, db, func(tx *sql.Tx) error {
		if err := repo.CreateCert(context.Background(), tx, cert); err != nil {
			return err
		}
...
func TestObserver_CertDivergence_MissingFullchain(t *testing.T) {
	obs, probe, repo, _, clock, c, cleanup := newTestNetObserver(t)
	defer cleanup()

	certID := seedAppliedCert(t, c.db, repo, "x.local", clock.Now().Add(90*24*time.Hour))

	probe.setResult(ProbeResult{
		ProbedAt: clock.Now().UTC(),
		Certs: []ProbedCert{{
			ID: certID, Domain: "x.local",
			FullchainExists: false, PrivkeyExists: true,
		}},
	})
	...
	snap := obs.Snapshot()
	if len(snap.CertDivergences) != 1 {
		t.Fatalf("cert divergences = %d", len(snap.CertDivergences))
	}
	if snap.CertDivergences[0].Reason != "missing_fullchain" {
...
func TestObserver_CertDivergence_Expired(t *testing.T) {
	obs, probe, repo, _, clock, c, cleanup := newTestNetObserver(t)
	defer cleanup()

	expired := clock.Now().Add(-time.Hour) // ya caducó
	certID := seedAppliedCert(t, c.db, repo, "x.local", expired)

	probe.setResult(ProbeResult{
		ProbedAt: clock.Now().UTC(),
		Certs: []ProbedCert{{
			ID: certID, Domain: "x.local",
			FullchainExists: true, PrivkeyExists: true,
			NotBefore: expired.Add(-365 * 24 * time.Hour),
			NotAfter:  expired,
		}},
	})
...
func TestObserver_CertDivergence_NotAfterMismatch(t *testing.T) {
	obs, probe, repo, _, clock, c, cleanup := newTestNetObserver(t)
	defer cleanup()

	dbNotAfter := clock.Now().Add(90 * 24 * time.Hour)
	diskNotAfter := clock.Now().Add(180 * 24 * time.Hour) // disco renovado externamente
	certID := seedAppliedCert(t, c.db, repo, "x.local", dbNotAfter)
	...
	snap := obs.Snapshot()
	if len(snap.CertDivergences) != 1 || snap.CertDivergences[0].Reason != "not_after_mismatch" {
		t.Errorf("divergences = %+v", snap.CertDivergences)
	}
}

func TestObserver_CertExpiringNotDivergence(t *testing.T) {
	obs, probe, repo, _, clock, c, cleanup := newTestNetObserver(t)
	defer cleanup()

	// Cert expira en 10 días — dentro de la ventana de "expiring" (30d).
	notAfter := clock.Now().Add(10 * 24 * time.Hour)
	certID := seedAppliedCert(t, c.db, repo, "soon.local", notAfter)

	probe.setResult(ProbeResult{
		ProbedAt: clock.Now().UTC(),
		Certs: []ProbedCert{{
			ID: certID, Domain: "soon.local",
			FullchainExists: true, PrivkeyExists: true,
			NotBefore: notAfter.Add(-365 * 24 * time.Hour),
			NotAfter:  notAfter,
		}},
	...
	snap := obs.Snapshot()
	if snap.DivergenceCount != 0 {
		t.Errorf("expiring should NOT be divergence; got %d", snap.DivergenceCount)
	}
	if snap.CertsExpiring != 1 {
